...
function List({ devices, onShow, setMsg, msg, onNewScanResult }: any) {
  ...
  const unassignDevice = async (id: string) => {
    try {
      await restService.unassignDeviceById(id).then(()=>{
  ...
  return (
    <div>
      {onShow === true ? (
        ...
        <div
          className={`${classes._list_container} border border-stone-900 border-solid rounded-md _fc`}
        >
          {/* #LFC */}
          <div>
            <div
              className={`flex items-center border-b-4 border-double border-slate-700 pl-2 gap-2 _bg-trans h-10`}
            >
              <svg
            ...
            <ul className={classes.device_container}>
              {devices
                ...
                .map(({ model, deviceId }: any) => (
                  <li
                    key={deviceId}
                    className={`text-slate-200 hover:bg-sky-700 flex items-center h-8 p-2 ${
                      match1100 && "text-sm"
                    }`}
          ...
          <div>
            {icon && (
              <>
                <div
                  className={`w-1/4 flex flex-col items-center justify-center gap-2 ml-2 ${
                    match750 && "my-1"
                  }`}
                >
                  <Image
                    priority
                    src={`/assets/${icon.match(/\S/g).join('')}.svg`}
                    alt={icon}
                    width={250}
                    height={250}
                ...
                <div
                  className={`flex flex-col gap-1 items-center justify-center ${
                    match750 ? "px-1 w-full" : "px-7 w-2/4 "
                  }`}
                >
                  {deviceName && (
                    <>
                      ...
                      {match1100 && (
                        <>
                          ...
                          <p className={classes.tpy}>
                            ...
                            <span>{formatFireBaseDate(purchaseDate.seconds,purchaseDate.nanoseconds)}</span>
                          </p>
                          <p className={classes.tpy}>
                            <span>Warranty Expiry :</span>
                            <span>{formatFireBaseDate(warrantyExpiry.seconds,warrantyExpiry.nanoseconds)}</span>
                          </p>
                        </>
                      )}
                    </>
                ...
                <div
                  className={`flex flex-col justify-between items-end  gap-1 ${
                    match750 ? " w-full" : "w-1/4 pb-2"
                  }`}
                >
