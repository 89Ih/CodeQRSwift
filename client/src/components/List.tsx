...
function List({ devices, onShow, setMsg, msg, onNewScanResult }: any) {
  ...
  const { deviceId, icon, purchaseDate, warrantyExpiry, status, serial_number, deviceName, manufacturer, model } = deviceSet;
            ...
            <ul className={classes.device_container}>
              {devices
                ...
                .map(({ model, deviceId }: any) => (
                  <li key={deviceId} className={`text-slate-200 hover:bg-sky-700 flex items-center h-8 p-2 ${match1100
                    && 'text-sm'}`} onClick={() => retrieveById(deviceId)}
                  >
                    <h5>{model}</h5>
                  </li>
                ))}
            ...
            {icon && (
              <>
                <div className={`w-1/4 flex flex-col items-center justify-center gap-2 ml-2 ${match750 && 'my-1'}`}>
                  <Image priority src={`/assets/${icon}.svg`} alt={icon} width={250} height={250}
                ...
                <div className={`flex flex-col justify-between items-end  gap-1 ${match750 ? ' w-full' : 'w-1/4 pb-2'}`}>
                  <Toolbar clss="flex p-2 gap-1">
                    <Toolbar.Iconbar alt="Delete" src="/assets/Delete.svg" onClick={() => unassignDevice(deviceId)}
                    />
                  </Toolbar>
                  {
                    !match1100 &&
                    <div style={{ width: "230px" }}>
                      <div className="flex gap-2">
                        <Image width={22.5} height={22.5} src={`/assets/Status.svg`} alt="Status" />
                      ...
                      <div className="flex gap-2">
                        <Image width={22.5} height={22.5} src={`/assets/PurchaseDate.svg`} alt="Purchase Date" />
                        <p className="text-sm flex gap-1 items-center justify-center h-6">
                          <b>Purchase Date :</b>
                          <span>{purchaseDate}</span>
                        </p>
                      ...
                      <div className="flex gap-2">
                        <Image width={22.5} height={22.5} src={`/assets/WarrantyExpiry.svg`} alt="Warranty Expiry" />
                        <p className="text-sm flex gap-1 items-center justify-center h-6">
                          <b>Warranty Expiry :</b>
                          <span>{warrantyExpiry}</span>
                        </p>
